...
class Squat(BaseExercise):
    ...
    def process(self, landmarks: List[List[int]]) -> Tuple[int, str, float, str, bool, str, float]:
        ...
        if ankle_dist > 0 and (knee_dist / ankle_dist) < 0.70:
        ...
        if not self.active:
            self.active = True
            self.stage = "UP"
            self.feedback = "TRACKING ACTIVE! SQUAT DOWN TO PARALLEL"
            if not self.start_workout_time:
                self.start_workout_time = time.time()

        if angle <= 125:
            if self.stage == "UP" or not self.has_reached_target_state:
                self.stage = "DOWN"
